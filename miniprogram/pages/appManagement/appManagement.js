    ...
    for (let i = 0; i < flpa.length; i++) {
      ...
      wx.cloud.uploadFile({
        ...
        success: function(res) {
          ...
          that.setData({
            imgUrl: imgArray, //云存储图片路径,可以把这个路径存到集合，要用的时候再取出来
          })
    ...
    if (this.data.idInColl && this.data.title) {
      ...
      db.collection(that.data.targetCollection[that.data.targetCollectionIndex]).doc(that.data.idInColl).update({
        ...
        success(res) {
          ...
          //discover collection index is 0
          // var str_imgUrl = JSON.stringify(that.data.imgUrl)
          if (that.data.targetCollectionIndex == 0) {
            ...
            db.collection('giftUser').add({
              data: {
                giftList_ID: that.data.idInColl,
                createDate: new Date(),
                participators: [],
              },
              success(res) {
